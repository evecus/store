...
func (s *Server) BootstrapAdmin() error {
	if s.Cfg.AdminPassword == "admin" {
		log.Printf("WARNING: admin account %q is using the default password \"admin\" — set the `password` environment variable to a strong value before exposing this server to the network", s.Cfg.AdminUsername)
	}
